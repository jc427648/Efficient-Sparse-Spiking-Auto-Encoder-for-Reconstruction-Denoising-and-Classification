...
def PlotError(error,title):
    ...
    ax = sns.heatmap(error.detach().numpy(),vmin = -1, vmax = 1,annot=True,fmt = "0.2f",cmap ="YlGnBu",cbar_kws={"label":"Scale"})
    plt.savefig(title)
    plt.close()
...
def PlotWeights(W1,W2,title1,title2,num_neuron):
    ...
    plt.imshow(W2_RS, vmin = 0, vmax = 1, cmap = "hot_r")
    plt2.savefig(title2)
    plt.close(plt2)
    # if clust == True:
    #     W3 = torch.reshape(W3,(28*5,28*2))
    #     plt3 = plt.figure()
    #     plt.title('W3')
    #     plt.imshow(W3,vmin=0,vmax=1,cmap = "hot_r")
    #     plt3.savefig()
    #     plt.close(plt3)
...
def PlotRaster(data,title):
    ...
    plt.imshow(data,vmin = 0, vmax = data.max(), cmap = "hot_r",aspect="auto")
    plt.colorbar()
    plt.savefig(title)
    plt.close()
