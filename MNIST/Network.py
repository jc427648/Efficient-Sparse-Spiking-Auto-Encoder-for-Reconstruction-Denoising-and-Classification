...
class Net(nn.Module):
    def __init__(self,
        ...
        self.lif1 = snn.Leaky(beta = beta1,
                              ...
                              threshold = torch.ones(num_hidden),
                              inhibition=True)
        self.lif2 = snn.Leaky(beta = beta2,
                              learn_threshold=True,
                              threshold = torch.ones(num_inputs),
                                inhibition = False) #not sure if should be true or false.
        ...
        self.HidActivity = torch.zeros(num_hidden) 
        self.OutActivity = torch.zeros(num_inputs)       
        InitThresh1 = 100*torch.ones(num_hidden)
        InitThresh2 = 2.0*torch.ones(num_inputs)
        self.clust  = False
        self.w3Update = True
    ...
    def CalcError(self,spk2_recon,image):
        ...
        error = (image-spk2_recon)/200#Should be image time.
    ...
    def PresentImage(self,image,time,pw,UpdateParams=True):
        ...
        self.mem3 = torch.zeros(self.num_class)
        ...
        for t  in range(time):
            ...
            # #Experimenting with refractory periods.
            # if torch.sum(spk1)>0:
            #      mem1 = -10*torch.ones_like(mem1)

            # if torch.sum(spk3)>0:
            #     self.mem3 = -10*torch.ones_like(self.mem3)

            if UpdateParams == True:
                 
                NewThresh1 = self.lif1.threshold.data - self.decay1 + self.increase1*spk1
                # NewThresh2 = self.lif2.threshold.data - self.decay2 +self.increase2*spk2#Comment out for better results.
                ...
                # NewThresh2.clamp_(self.lif2Min,self.lif2Max)
                with torch.no_grad():
                    self.lif1.threshold.copy_(NewThresh1.squeeze())
                    # self.lif2.threshold.copy_(NewThresh2.squeeze())#comment out for better results.
        ...
        if UpdateParams == True:
            ...
            # WD = self.fc1.weight.detach()*(1-self.fc1.weight.detach()) #Weight dependancy
            # delta_w1.mul_(WD)
            NewWeights1 = (delta_w1 + self.fc1.weight.detach()).detach().clamp(0,1) #We're simulating weights between 0 and 1
            Syn1_count = Syn1_count + torch.sum((delta_w1!=0))
            
            
            
            # delta_w1.mul_((1-delta_w))           
            delta_w2.mul_(self.rho.unsqueeze(1).repeat(1,self.num_hidden)*Relevance.repeat(self.num_inputs,1))#Comment out for ablation study
            Syn2_count = Syn2_count + torch.sum((delta_w2!=0))
        ...
        error, error_scalar = self.CalcError(spk2_recon=spk2_recon,image=image)
        # print(self.rho)
        if (self.w3Update==True):
            ...
            #Adding weight dependancy (just trialling)
            # delta_w3  = delta_w3*(1-self.fc3.weight.detach())#Ignored mu**1#mu is equal to 1 in this instance.
            #Should be self.fc3.weight.detach()!!!
            # print(delta_w3)
            NewWeights3 = torch.add(delta_w3,self.fc3.weight.detach())
            # WD = self.fc3.weight.detach()*(1-self.fc3.weight.detach())
            # delta_w3.mul_(WD)
            NewWeights3.clamp_(0,1)#Commented for soft bound weight dependancy
            Syn3_count = Syn3_count + torch.sum((delta_w3!=0))
        ...
        return error, error_scalar, spk3_recon, spk2_recon, spk1_recon, Syn1_count, Syn2_count, Syn3_count
